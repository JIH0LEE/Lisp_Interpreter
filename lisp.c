...
static object *create_object(int type) /* type에 해당하는 objcet 생성  */
{
...
#define cadr(obj)	car(cdr(obj)) /* car cdr 혼합사용 명령어 정의 */
...
static object *prim_list(object *env, object *args) /* list 생성 */
{
	return args;
}
...
static void define_prim(object *env) /* 입력에 대해 해당하는 함수호출 */
{
